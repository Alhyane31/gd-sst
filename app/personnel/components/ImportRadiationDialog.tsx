...
const TEMPLATE_HEADERS = "matricule;dateSortie";
const TEMPLATE_EXAMPLE = "M001;31/12/2024";
...
function downloadTemplate() {
  ...
  a.href = url; a.download = "canevas_import_radiation.csv";
  document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
}

type Props = { open: boolean; onClose: () => void; onSuccess: () => void };

export default function ImportRadiationDialog({ open, onClose, onSuccess }: Props) {
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await fetch("/api/personnel/import-radiation", {
        method: "POST",
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
      <DialogTitle>Importer des radiations</DialogTitle>
      <DialogContent>
        <Stack spacing={2} mt={1}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 2, flexWrap: "wrap" }}>
            ...
            <Typography variant="caption" color="text.secondary">
              Colonnes : {TEMPLATE_HEADERS} — Date au format DD/MM/YYYY
            </Typography>
          </Box>
          ...
          {result && (
            <Box>
              <Stack direction="row" spacing={1} mb={1}>
                <Chip label={`${result.ok.length} radiés`}    color="success" size="small" />
              ...
              {tab === 0 && (
                <Box sx={{ maxHeight: 300, overflow: "auto" }}>
                  <Table size="small">
                    <TableHead>
                      <TableRow><TableCell>#</TableCell><TableCell>Matricule</TableCell><TableCell>Nom</TableCell><TableCell>Date sortie</TableCell></TableRow>
                    </TableHead>
                    <TableBody>
                      {result.ok.map((r) => (
                        <TableRow key={r.ligne}>
                          ...
                          <TableCell>{r.dateSortie}</TableCell>
                        </TableRow>
                      ))}
                      {result.ok.length === 0 && <TableRow><TableCell colSpan={4} align="center">Aucune radiation effectuée</TableCell></TableRow>}
                    </TableBody>
                  </Table>
                </Box>
              )}
              {tab === 1 && (
                <Box sx={{ maxHeight: 300, overflow: "auto" }}>
                  <Table size="small">
                    <TableHead>
                      <TableRow><TableCell>#</TableCell><TableCell>Matricule</TableCell><TableCell>Raison</TableCell></TableRow>
                    </TableHead>
                    <TableBody>
                      {result.errors.map((r) => (
                        <TableRow key={r.ligne}>
                      ...
                      {result.errors.length === 0 && <TableRow><TableCell colSpan={3} align="center">Aucune erreur</TableCell></TableRow>}
                    </TableBody>
                  </Table>
                </Box>
              )}
      ...
      <DialogActions>
        ...
        {!result && (
          <Button variant="contained" color="error" onClick={handleImport} disabled={rows.length === 0 || loading}>
            {loading ? "Traitement..." : `Valider et radier (${rows.length} ligne(s))`}
          </Button>
        )}
        {result && (
